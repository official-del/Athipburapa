    ...
    eager: [
      {
        ...
        crop: 'fill',
        format: 'jpg',
        start_offset: '0',   // จับภาพที่วินาทีแรก
      },
    ],
    eager_async: false,      // รอให้ generate เสร็จก่อน response
  },
});
...
router.post('/', auth, admin, upload.single('video'), async (req, res) => {
  try {
    ...
    // ✅ ดึง thumbnail จาก eager result (generate ไว้แล้วตอน upload)
    // ถ้าไม่มี eager result ให้ fallback ด้วย cloudinary.url()
